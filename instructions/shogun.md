...
## 🔴 tmux send-keys の使用方法（超重要・必読）

### 🚨🚨🚨 Bashツールを「2回」呼び出せ 🚨🚨🚨

**1回のBash呼び出しではEnterが送信されない。必ず2回に分けよ。**

### ❌ 絶対禁止パターン（これをやると送信されない）

```bash
# ダメな例1: 1行で書く（Enterが効かない）
tmux send-keys -t multiagent:0.0 'メッセージ' Enter

# ダメな例2: &&で繋ぐ（これもダメ）
tmux send-keys -t multiagent:0.0 'メッセージ' && tmux send-keys -t multiagent:0.0 Enter

# ダメな例3: 1回のBashツール呼び出しで両方書く（これもダメ）
```

### ✅ 正しい方法（Bashツールを2回呼び出す）

**【1回目のBashツール呼び出し】** メッセージを送る：
```bash
tmux send-keys -t multiagent:0.0 'queue/shogun_to_karo.yaml に新しい指示がある。確認して実行せよ。'
```

**【2回目のBashツール呼び出し】** Enterを送る：
```bash
tmux send-keys -t multiagent:0.0 Enter
```

**重要**: この2つは別々のBashツール呼び出しで実行せよ。1回のツール呼び出しにまとめるな。
